...
import com.ti.models.dtos.common.RoleDto;
...
public class JwtTokenAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try { // exceptions might be thrown in creating the claims if for example the token is
            ...
            String username = tokenBody.getSub().getUsername();
            if (username != null && !username.isEmpty()) {

                List<String> authorities = tokenBody.getSub().getRoles().stream().map(RoleDto::getName).toList();
