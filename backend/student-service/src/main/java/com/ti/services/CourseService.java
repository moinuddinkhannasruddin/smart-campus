...
import com.ti.exceptions.errors.CourseErrors;
...
import com.ti.repositories.CoursesRepository;
import com.ti.repositories.specification.CourseSpecification;
import com.ti.repositories.specification.SearchCriteria;
...
public class CourseService {
    ...
    public Page<CourseResponse> getCourses(Pageable pageable,Integer
        ...
        CourseSpecification cs = new CourseSpecification(new SearchCriteria("entityId", ":", entityId));

        Specification<Course> spec = Specification.where(cs);

        if(subEntityId != null){
            spec = spec.and(new CourseSpecification(new SearchCriteria("subEntityId", ":", subEntityId)));
        }

        if(userId != null && !userId.isEmpty()){
            spec = spec.and(new CourseSpecification(new SearchCriteria("userId", ":", userId)));
        }

        if(courseCode!= null && !courseCode.isEmpty()){
            spec = spec.and(new CourseSpecification(new SearchCriteria("courseCode", ":", courseCode)));
        }

        if(courseName != null && !courseName.isEmpty()){
            spec = spec.and(new CourseSpecification(new SearchCriteria("courseName", ":", courseName)));
        }

        if(courseType != null && !courseType.isEmpty()){
            spec = spec.and(new CourseSpecification(new SearchCriteria("type", ":", courseType)));
        }

        Page<Course> courses = coursesRepository.findAll(spec, pageable);

        return CourseResponse.toPageDto(courses);
    }
}
